...
- [ ] **Lifecycle hooks + notification delivery** (#5) — this module's `notifyEvent`
  (append to `events/events.jsonl` + dashboard wake hint) is the single choke
  point every state transition flows through. A hook system taps it here: skill-declared code on transitions,
